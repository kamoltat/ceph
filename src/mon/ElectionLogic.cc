...
{
  if (strategy == CLASSIC) {
      ldout(cct, 5) << "defer to " << who << " (mon." << elector->get_rank_name(who) << ")" << dendl;
      ceph_assert(who < elector->get_my_rank());
  } else {
    ldout(cct, 5) << "defer to " << who << " (mon." << elector->get_rank_name(who) << "), disallowed_leaders=" << elector->get_disallowed_leaders() << dendl;
